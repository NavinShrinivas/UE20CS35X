...
async def run(loop):
    BEST_HACKATHON = raftos.Replicated(name='BEST_HACKATHON')
    ...
    with open(NODE_LOGFILE, 'a') as log_file:
        ...
        # the variable 'BEST_HACKATHON' to something specific

        # This specific code initially sets the value to
        # 'somerandomhackathon' if it hasn't been set before,
        # and to the next value once the next leader enters
        # this block of code and realises that the value
        # has already been set before
        try:
            old_value = await BEST_HACKATHON.get()
            if old_value is not None:
                await BEST_HACKATHON.set('HashCode 11.0; Go register!')
            else:
                await BEST_HACKATHON.set('somerandomhackathon') 
        except:
            await BEST_HACKATHON.set('somerandomhackathon')

        # Client Command executed;    
        log_file.write('Done writing BEST_HACKATHON\n Check the .log and .state_machine files!')
