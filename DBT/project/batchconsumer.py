...
# Batch : 
df = spark.read.format("kafka").option("kafka.bootstrap.servers", "localhost:9092").option("startingOffsets", "earliest").option("subscribe","test").option("includeTimestamp","true").load()
query = df.select((df.key).alias("HashTag"),(df.timestamp).alias("time")).groupBy(window("time","20 seconds"),"HashTag").count().sort(desc("window"))
query = query.selectExpr("CAST(window AS STRING)","CAST(HashTag AS STRING)", "CAST(count AS STRING)")
query.show()
# query.writeStream.outputMode("complete").trigger(processingTime="20 seconds").format("console").option("truncate","false").start().awaitTermination()
#
